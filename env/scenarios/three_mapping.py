...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        world.agents[2].color = np.array([0.25, 0.25, 0.75])
        ...
        world.landmarks[2].state.p_pos = np.array([-5, 5])
    ...
    def reward(self, agent, world):
        ...
        flag = True
        for a_idx in ([0,1],[0,2],[1,2]):
            if np.sqrt(np.sum(np.square(world.agents[a_idx[0]].state.p_pos - world.agents[a_idx[1]].state.p_pos)))<4:
                flag = False       
        if np.all(np.array([min(dists[i])<np.sqrt(2) for i in range(world.num_agents)])) and flag:
            reward = 50
        else:
            reward = sum([10 / (min(dists[i])+0.1) for i in range(world.num_agents)])
    ...
    def done(self,agent,world):
        ...
        # if min(dists[0])<0.1*np.sqrt(2) and min(dists[1])<np.sqrt(2) and np.sqrt(np.sum(np.square(world.agents[0].state.p_pos - world.agents[1].state.p_pos)))>4:
        #     done = True
        # else:
        #     done = False
        flag = True
